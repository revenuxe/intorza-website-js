...
import { CityLocalBusinessSchema, CitySoftwareSchema } from "@/components/seo/CitySEOSchema";

type Props = {
  params: Promise<{ countryName: string; citySlug: string }>;
};

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { countryName, citySlug } = await params;
  const city = getCityBySlug(countryName, citySlug);
  const country = countries.find(c => c.slug === countryName.toLowerCase());
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `Intorza Interior Design Software in ${city.name}`,
        },
      ],
      type: "website",
    },
...
export default async function CityPage({ params }: Props) {
  ...
  const breadcrumbItems = [
    { name: country.name, url: `${baseUrl}/${country.slug}` },
  ...
  return (
    <>
      <CityLocalBusinessSchema city={city} country={country} />
      <CitySoftwareSchema city={city} country={country} />
      <Script
